...
# set the log level of logging to INFO
...
def train_val_split(data: pd.DataFrame, 
    ...
    dataset_logger.save_logs(msg=f'Data is split into train split with shape {train_data.shape} and val split with shape {val_data.shape}', log_level='info')
...
def read_params(params_path: str) -> int:
    try:
        with open(params_path) as f:
            params_file = safe_load(f)
    except FileNotFoundError as e:
          dataset_logger.save_logs(msg='Parameters file not found, Switching to default value for train test split', log_level='error')
